...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_telemost() {
            if s.contains("Telemost")
                && !name.starts_with("upgrade_telemost_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("Telemost") {
                    s = s.replace("Telemost", &app_name);
                } else {
                    // https://github.com/rustdesk/rustdesk-server-pro/issues/845
                    // If app_name contains "Telemost" (e.g., "Telemost-Admin"), we need to avoid
                    // replacing "Telemost" within the already-substituted app_name, which would
                    // cause duplication like "Telemost-Admin" -> "Telemost-Admin-Admin".
                    //
