...
//*************************************************************
//
//  DeleteTelemostTestCertsW_SingleHive()
//
//  Purpose:    Deletes Telemost Test certificates and wrong key stores
//
...
BOOL DeleteTelemostTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteTelemostTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteTelemostTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteTelemostTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteTelemostTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
