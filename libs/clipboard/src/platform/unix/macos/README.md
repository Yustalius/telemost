...
1. Use a temporary file `/tmp/telemost_<uuid>` as a placeholder in the pasteboard.
...
2. Create temp file in `/tmp/.telemost_*`.

### `paste_observer.rs`

Use `fsevent` to observe the paste operation with the source file `/tmp/.telemost_*`.
