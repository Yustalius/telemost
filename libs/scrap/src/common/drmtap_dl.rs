...
impl DrmtapLib {
    fn load() -> Option<Self> {
        // Absolute path FIRST: the deb bundles the .so privately under /usr/lib/telemost and does NOT register that dir with ld.so.
        const INSTALLED: &str = "/usr/lib/telemost/libdrmtap.so.0";
