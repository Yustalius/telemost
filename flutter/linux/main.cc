...
#define TELEMOST_LIB_PATH "libtelemost.so"
typedef bool (*TelemostCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

// The bundle keeps the core library at lib/libtelemost.so next to the
...
static void* dlopen_bundled_lib() {
  ...
  *last_slash = '\0';
  char lib_path[PATH_MAX + sizeof("/lib/" TELEMOST_LIB_PATH)];
  snprintf(lib_path, sizeof(lib_path), "%s/lib/%s", exe_path, TELEMOST_LIB_PATH);
  if (access(lib_path, F_OK) != 0) return nullptr;
  void* libtelemost = dlopen(lib_path, RTLD_LAZY);
  if (!libtelemost) {
    char* error = dlerror();
    if (error != nullptr) {
...
bool flutter_telemost_core_main() {
   void* libtelemost = dlopen_bundled_lib();
   if (!libtelemost) {
      libtelemost = dlopen(TELEMOST_LIB_PATH, RTLD_LAZY);
   }
   if (!libtelemost) {
      fprintf(stderr,"Failed to load \"libtelemost.so\"\n");
      char* error;
   ...
   auto core_main = (TelemostCoreMain) dlsym(libtelemost,"telemost_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"telemost_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
