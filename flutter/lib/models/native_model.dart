...
/// The Linux bundle keeps the core library at lib/libtelemost.so next to the
...
DynamicLibrary _openLinuxCoreLib() {
  final bundled =
      '${File(Platform.resolvedExecutable).parent.path}/lib/libtelemost.so';
  try {
  ...
  return DynamicLibrary.open('libtelemost.so');
}
...
class PlatformFFI {
  ...
  TelemostImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libtelemost.so')
        : isLinux
            ? _openLinuxCoreLib()
            : isWindows
                ? DynamicLibrary.open('libtelemost.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("liblibtelemost.dylib") :
  ...
  void _startListenEvent(TelemostImpl telemostImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = telemostImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
